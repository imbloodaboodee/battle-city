...
public class PlayerTankControl extends JPanel {
    ...
    private Timer bulletCreation;

    public PlayerTankControl() {
        tank = new Tank();
        ...
        this.addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                ...
                bulletCreation.start(); // Start firing bullets
            ...
            public void mouseReleased(MouseEvent e) {
                ...
                bulletCreation.stop(); // Stop firing bullets

            }

        });
        bulletCreation = new Timer(0, e -> createBullet());
        bulletCreation.setRepeats(true);

        bulletTimer = new Timer(0, e -> updateBullets());
        bulletTimer.start();
    ...
    private void createBullet() {
        ...
        Bullet bullet = new Bullet(cannonTipX, cannonTipY, 1, 5, angle ); // Adjust angle for correct firing direction
        bullets.add(bullet);
    }
    ...
    private void updateBullets() {
        for (int i = 0; i < bullets.size(); i++) {
            ...
            if (bullets.get(i).getX() < 0 || bullets.get(i).getX() > getWidth() || bullets.get(i).getY() < 0 || bullets.get(i).getY() > getHeight()) {
                bullets.remove(i); // Remove bullet if it goes out of bounds
